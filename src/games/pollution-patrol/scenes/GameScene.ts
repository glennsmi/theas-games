...
export class GameScene extends Phaser.Scene {
  ...
  private bonusTrash!: Phaser.Physics.Arcade.Group
  ...
  private trashTimer!: Phaser.Time.TimerEvent
  ...
  create() {
    ...
    this.bonusTrash = this.physics.add.group()
    
    // Collisions
    this.physics.add.overlap(this.thea, this.trash, this.collectTrash as Phaser.Types.Physics.Arcade.ArcadePhysicsCallback, undefined, this)
    this.physics.add.overlap(this.thea, this.bonusTrash, this.collectBonusTrash as Phaser.Types.Physics.Arcade.ArcadePhysicsCallback, undefined, this)
    ...
    // Occasionally spawn bonus trash (cans)
    this.time.addEvent({
      delay: 8000,
      callback: this.spawnBonusTrash,
      callbackScope: this,
      loop: true
    })
  ...
  createHUD() {
    ...
    // Trash counter
    const trashIcon = this.add.sprite(width - 200, 30, 'plasticBottle')
    trashIcon.setScrollFactor(0)
    trashIcon.setDepth(100)
    trashIcon.setScale(0.8)
    
    this.scoreText = this.add.text(width - 180, 30, '0', {
  ...
  spawnTrash() {
    ...
    const item = this.trash.create(width + 20, y, 'plasticBottle') as Phaser.Physics.Arcade.Sprite
    item.setVelocityX(-this.gameSpeed)
    const itemBody = item.body as Phaser.Physics.Arcade.Body
    if (itemBody) itemBody.allowGravity = false
    ...
    if (Math.random() > 0.6) {
      for (let i = 1; i < 4; i++) {
        const clusterItem = this.trash.create(width + 20 + i * 25, y + (Math.random() - 0.5) * 40, 'plasticBottle') as Phaser.Physics.Arcade.Sprite
        clusterItem.setVelocityX(-this.gameSpeed)
        const clusterBody = clusterItem.body as Phaser.Physics.Arcade.Body
        if (clusterBody) clusterBody.allowGravity = false
        clusterItem.setAngle(Phaser.Math.Between(-20, 20))
      }
    }
  }
  ...
  spawnBonusTrash() {
    ...
    const item = this.bonusTrash.create(width + 20, y, 'sodaCan') as Phaser.Physics.Arcade.Sprite
    item.setVelocityX(-this.gameSpeed)
    const itemBody = item.body as Phaser.Physics.Arcade.Body
    if (itemBody) itemBody.allowGravity = false
    item.setScale(1.2)
    
    // Sparkle effect to show it's bonus
    this.tweens.add({
      targets: item,
      angle: 360,
      duration: 2000,
  ...
  collectTrash(_thea: Phaser.Types.Physics.Arcade.GameObjectWithBody | Phaser.Tilemaps.Tile, item: Phaser.Types.Physics.Arcade.GameObjectWithBody | Phaser.Tilemaps.Tile) {
    ...
    itemSprite.body?.enable && (itemSprite.body.enable = false)
    ...
    this.itemsCollected++
    this.scoreText.setText(this.itemsCollected.toString())
  ...
  collectBonusTrash(_thea: Phaser.Types.Physics.Arcade.GameObjectWithBody | Phaser.Tilemaps.Tile, item: Phaser.Types.Physics.Arcade.GameObjectWithBody | Phaser.Tilemaps.Tile) {
    const itemSprite = item as Phaser.Physics.Arcade.Sprite
  ...
  gameOver() {
    ...
    this.tweens.add({
      ...
      onComplete: () => {
        ...
        if (onGameOver) {
          onGameOver(this.distance, this.itemsCollected)
        }
      }
    })
  ...
  update(_time: number, delta: number) {
    ...
    // Update obstacle timer frequency based on speed
    ...
    this.trash.getChildren().slice().forEach((item) => {
      const p = item as Phaser.Physics.Arcade.Sprite
      if (!p.active) return
      if (p.x < -50) {
        p.destroy()
        return
    ...
    this.bonusTrash.getChildren().slice().forEach((item) => {
